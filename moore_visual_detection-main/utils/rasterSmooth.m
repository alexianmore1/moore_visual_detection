function  [ bins2 ,mean_rate ,m_FR] = arifRasterSmooth(time,event_matrix,plot_type,color)



if size(event_matrix,2)==1 && any(isnan(event_matrix))
else
%spike_times=nan;
for trial=1:size(event_matrix,2)
spike_times{trial} = (time((event_matrix(:,trial)==1)));
end


bin_size = .1;% 100ms
bins=[time(1):bin_size:time(end)];
bins2=bins(1:end-1)';

m_FR = nan;
for bn = 1:length(bins)-1
    for trial=1:size(event_matrix,2)
        m_FR(bn,trial) = length(find(spike_times{trial}>bins(bn) & spike_times{trial}<bins(bn+1)));
end  ;end

mean_rate  = ((mean(m_FR,2)/bin_size));

if plot_type==1 % actual raster
plotSpikeRaster2(spike_times,color,'PlotType','vertline');ylabel('Trial Number')
xlim([time(1) time(end)])
     ax=axis;line([0 0],[ax(3) ax(4)],'color','r','linewidth', 2,'linestyle','--');

% myfigurelayout1
elseif plot_type==2 % show the mean averaged
%     bar(bins(1:end-1),mean_rate./250,1,'facecolor','k','edgecolor','none');alpha(.7);ylabel('Licks/s');
%     int = mean_rate(find(bins>=0,1,'first'))/250;
%     ax=axis;line([ax(1) ax(2)],[int int],'color','k','linewidth', 1,'linestyle','--');
%      ax=axis;line([0 0],[ax(3) ax(4)],'color','r','linewidth', 2,'linestyle','--');
...
% ax=axis;line([0 0],[ax(3) ax(4)],'color','k','linewidth', 1.5);
...
function [xPoints, yPoints] = plotSpikeRaster2(spikes,color,varargin)
